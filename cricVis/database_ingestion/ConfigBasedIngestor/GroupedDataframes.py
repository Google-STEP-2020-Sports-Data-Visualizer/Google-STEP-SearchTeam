...
class GroupedDataframes:
    """Groups/Indexes the dataframes corresponding to each schema in database."""
    ...
    def __init__(self, final_dfs, configs):
        ...
        configs: dict
            Dictionary containing the config settings for each dataframe.

        initial_dfs: dict
            Dictionary of the dataframes created by the FinalDataframes module.
        ...
        final_dfs: dict
            Dictionary of dataframes corresponding to each schema.

        dfs: dict
            Dictionary of dataframes created after grouping/indexing all final_dfs.
        """
    ...
    def __group_df(self, dfname, df_config):
        """
        Creates the grouped/indexed dataframes and saves it in the dfs dictionary.
    ...
    def __execute(self, df, dfname, fn, fn_config):
        ...
        if fn == "copy":
            ...
            df[fn_config["column"]] = df[fn_config["column"]].apply(
                lambda x: util_fns.filter_rows(x, fn_config["values_allowed"]))
            ...
            dfs_subset_list = [self.__assign_based_on_class(
                fn_config["class"], subset_name) for subset_name in fn_config["dataframes"]]
            df = pd.concat(tuple(dfs_subset_list), ignore_index=True)

        elif fn == "merge":
            # Left table for making the join
            left = self.__assign_based_on_class(
                fn_config["left"]["class"], fn_config["left"]["dataframe"])
            left_cols = fn_config["left"]["columns"]

            # Right table for making the join
            right = self.__assign_based_on_class(
                fn_config["right"]["class"], fn_config["right"]["dataframe"])
            ...
            df.set_index(fn_config, inplace=True)
    ...
    def __assign_based_on_class(self, class_name, dataframe_name):
        ...
        if(class_name == "GroupedDataframes"):
            df_orig = self.dfs[dataframe_name]
        else:
            df_orig = self.final_dfs[dataframe_name]
